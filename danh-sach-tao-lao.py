...
def changeInfo(persons, database):
    ...
    while isRunning:
        ...
        try:
            ...
            update_data = {
                "$set": {
                    f"data.$[element].{changeName}": changeValue
                }
            }
            
            person_filters = [{"element.name": tmpName}]
            collection.update_one({}, update_data, array_filters=person_filters)
...
def delWithName(persons, deletedList, database):
    ...
    delete_data = {
        "$pull": {
            "data.deleted": tmpName
        }
    }
    
    # person_filter = [{"element.name": tmpName}]
    collection.update_one({},delete_data)
...
def DBRender(persons, database):
    ...
    for person in DBPersons:
        persons.append(Person(person["name"], person["age"], person["gender"], person["email"]))
...
while isRunning:
    ...
    # menu cấp 2 
        ...
        if choice == 1:
            ...
            clearTerminal()
            delWithName(persons, deletedList, database)
        elif choice == 3:
            clearTerminal()
            sortPersons(persons)
        elif choice == 4:
